...
namespace Manatee.Json.Serialization.Internal
{
	internal class AutoSerializer : ISerializer, ITypeSerializer
	{
		...
		public T Deserialize<T>(JsonValue json, JsonSerializer serializer)
		{
			...
			var propertyInfoList = GetProperties(type);
			var map = DeserializeValues(obj, json, serializer, propertyInfoList);
		...
		public void DeserializeType<T>(JsonValue json, JsonSerializer serializer)
		{
			var type = typeof (T);
			var propertyInfoList = GetTypeProperties(type);
			var map = DeserializeTypeValues(json, serializer, propertyInfoList);
			if ((json.Object.Count > 0) && (serializer.Options.InvalidPropertyKeyBehavior == InvalidPropertyKeyBehavior.ThrowException))
				throw new TypeDoesNotContainPropertyException(type, json);
		...
		private static Dictionary<PropertyInfo, object> DeserializeValues<T>(T obj, JsonValue json, JsonSerializer serializer, IEnumerable<PropertyInfo> properties)
		{
		...
		private static Dictionary<PropertyInfo, object> DeserializeTypeValues(JsonValue json, JsonSerializer serializer, IEnumerable<PropertyInfo> properties)
		{
			var dict = new Dictionary<PropertyInfo, object>();
			foreach (var propertyInfo in properties)
			{
